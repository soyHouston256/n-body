...
// Default to FOURTH order for linter/IDE visibility if not defined by Makefile
#if !defined(FOURTH) && !defined(SIXTH) && !defined(EIGHTH)
#define FOURTH
#endif
...
#define PROFILE
#ifdef PROFILE
extern double wtime() { return MPI_Wtime(); }
#else
extern double wtime() { return 0; }
#endif

static void get_CPU_time(double *time_real, double *time_user,
                         double *time_syst) {
  struct rusage xxx;
...
static void outputsnap(FILE *out) {
  ...
  for (int i = 0; i < nbody; i++) {
    ...
    fprintf(out, "%06d  %.8E  % .8E % .8E % .8E  % .8E % .8E % .8E \n", p.id,
            p.mass, p.pos[0], p.pos[1], p.pos[2], p.vel[0], p.vel[1], p.vel[2]);
  }
}

static void outputsnap() {
  static char out_fname[256];
  sprintf(out_fname, "outputs/%04d.dat", diskstep);
  FILE *out = fopen(out_fname, "w");
  assert(out);
  outputsnap(out);
  fclose(out);

  out = fopen("outputs/data.con", "w");
  assert(out);
  outputsnap(out);
  fclose(out);
}
...
static void energy(int myRank) {
  ...
  if (myRank == 0) {
    printf("%.4E   %.3E %.3E   % .6E % .6E % .6E % .6E   %.6E \n", time_cur,
           Timesteps, n_act_sum, E_pot, E_kin, E_pot + E_kin, eerr,
           CPU_time_user - CPU_time_user0);
    fflush(stdout);

    FILE *out = fopen("outputs/contr.dat", "a");
    assert(out);

    fprintf(out,
            "%.8E  %.8E  %.8E   % .16E % .16E % .16E   % .16E   % .16E % .16E  "
            " % .16E % .16E % .16E   %.8E %.8E %.8E \n",
            time_cur, Timesteps, n_act_sum, E_pot, E_kin, E_pot + E_kin, eerr,
            rcm_mod, vcm_mod, mom[0], mom[1], mom[2],
            CPU_time_real - CPU_time_real0, CPU_time_user - CPU_time_user0,
            CPU_time_syst - CPU_time_syst0);
    fclose(out);
  }
...
int main(int argc, char *argv[]) {
  ...
  if (myRank == 0) {
...
#ifdef FOURTH
      config_file = "phi-GPU4.cfg";
#endif
#ifdef SIXTH
      config_file = "phi-GPU6.cfg";
#endif
#ifdef EIGHTH
      config_file = "phi-GPU8.cfg";
#endif
    }
    ...
    printf("\n");
//		printf("Begin the calculation of phi-GPU program on %03d
// processors\n", n_proc);
#ifdef FOURTH
...
#pragma omp parallel for
    ...
    MPI_Allreduce(force_tmp, force, ni * Force::nword, MPI_DOUBLE, MPI_SUM,
                  MPI_COMM_WORLD);
  ...
  if (myRank == 0) {
    get_CPU_time(&CPU_time_real, &CPU_time_user, &CPU_time_syst);
    double Gflops = Particle::flops * 1.e-9 * Particle::init_iter *
                    double(nbody) * nbody / (CPU_time_real - CPU_time_real0);
  ...
  double t_scan = 0.0, t_pred = 0.0, t_jsend = 0.0, t_isend = 0.0,
         t_force = 0.0, t_recv = 0.0, t_comm = 0.0, t_corr = 0.0;
...
#pragma omp parallel for
  ...
  double g6_calls_sum;
  MPI_Reduce(&g6_calls, &g6_calls_sum, 1, MPI_DOUBLE, MPI_SUM, 0,
             MPI_COMM_WORLD);

  if (myRank == 0) {
    printf("\n");
    printf("Timesteps = %.0f   Total sum of integrated part. = %.0f   n_act "
           "average = %.0f\n",
           Timesteps, n_act_sum, n_act_sum / Timesteps);
    printf("\n");
    // double Gflops  = 57.0e-9*nbody*n_act_sum/(CPU_time_user -
    // CPU_time_user0); double GflopsT
    // = 57.0e-9*nbody*g6_calls*48/(CPU_time_user - CPU_time_user0);
    double Gflops = Particle::flops * 1.e-9 * double(nbody) *
                    double(n_act_sum) / (CPU_time_real - CPU_time_real0);
    // double GflopsT = Particle::flops*1.e-9*nbody*g6_calls*48/(CPU_time_real -
    // CPU_time_real0);
    ...
    // t_pred/t_tot, t_forc/t_tot, t_comm/t_tot, t_corr/t_tot);
    printf("        sec_tot     usec/step   ratio\n");
    printf("scan :%12.4E%12.4E%12.4E\n", t_scan, t_scan / Timesteps * 1.e6,
           t_scan / t_tot);
    printf("pred :%12.4E%12.4E%12.4E\n", t_pred, t_pred / Timesteps * 1.e6,
           t_pred / t_tot);
    printf("jsend:%12.4E%12.4E%12.4E\n", t_jsend, t_jsend / Timesteps * 1.e6,
           t_jsend / t_tot);
    printf("isend:%12.4E%12.4E%12.4E\n", t_isend, t_isend / Timesteps * 1.e6,
           t_isend / t_tot);
    printf("force:%12.4E%12.4E%12.4E\n", t_force, t_force / Timesteps * 1.e6,
           t_force / t_tot);
    printf("recv :%12.4E%12.4E%12.4E\n", t_recv, t_recv / Timesteps * 1.e6,
           t_recv / t_tot);
    printf("comm :%12.4E%12.4E%12.4E\n", t_comm, t_comm / Timesteps * 1.e6,
           t_comm / t_tot);
    printf("corr :%12.4E%12.4E%12.4E\n", t_corr, t_corr / Timesteps * 1.e6,
           t_corr / t_tot);
    printf("tot  :%12.4E%12.4E%12.4E\n", t_tot, t_tot / Timesteps * 1.e6,
           t_tot / t_tot);
