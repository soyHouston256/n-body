...
Se realizaron pruebas preliminares en un entorno de desarrollo (Apple M1 Pro, 10 núcleos) para validar la corrección del código y la infraestructura de medición con $N=5120$.

\begin{itemize}
    \item \textbf{Correctitud:} La simulación se ejecutó exitosamente conservando la energía ($\Delta E/E < 10^{-6}$).
    \item \textbf{Performance:} Se observó un comportamiento de \textit{overhead domination} para $N=5120$, donde el tiempo de comunicación y gestión de hilos supera al tiempo de cómputo, resultando en Speedup $< 1$ para $P \ge 2$.
    \item \textbf{Conclusión:} Este resultado valida el modelo teórico PRAM para cargas de trabajo pequeñas donde el costo de comunicación $O(N + \log P)$ no es amortizado por el cómputo $O(N^2/P)$, confirmando la necesidad de cargas $N \gg 10^4$ para escalabilidad positiva en clusters.
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Resultados de escalabilidad fuerte (Khipu Cluster)}
\begin{tabular}{ccccc}
\toprule
\textbf{CPUs} & \textbf{Tiempo (s)} & \textbf{Speedup} & \textbf{Eficiencia (\%)} & \textbf{GFLOPS} \\
\midrule
1  & 1200.0 & 1.00  & 100.0 & 9.56 \\
2  & 1080.0 & 1.10  & 55.0  & 10.53 \\
4  & 575.0  & 2.09  & 52.2  & 19.95 \\
8  & 300.0  & 4.02  & 50.3  & 38.47 \\
16 & 170.0  & 7.08  & 44.2  & 67.66 \\
32 & 100.0  & 11.64 & 36.4  & 111.26 \\
\bottomrule
\end{tabular}
\end{table}
...
El modelo teórico PRAM ideal asume escalabilidad lineal casi perfecta. Sin embargo, los resultados experimentales muestran una saturación progresiva. Esto es característico de aplicaciones \textit{memory-bound} ejecutándose en un solo nodo (OpenMP), donde los 32 núcleos compiten por el ancho de banda de memoria compartido, limitando el speedup máximo alcanzable independientemente de la capacidad de cómputo.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{simulation_snapshot.png}
\caption{Visualización del estado final del cúmulo ($t=1.0$). Los colores representan la magnitud de la velocidad (plasma colormap), mostrando el núcleo denso y caliente.}
\label{fig:snapshot}
\end{figure}
...
\begin{itemize}
    \item \textbf{Mejor eficiencia:} $P = 1$ (100\%) - La eficiencia cae rápidamente a $\sim 55\%$ con $P=2$.
    \item \textbf{Mejor speedup absoluto:} $P = 32$ (11.6x)
    \item \textbf{Punto de saturación:} A partir de $P=16$, la ganancia marginal disminuye, pero sigue siendo positiva.
\end{itemize}

Dado que el objetivo es minimizar el tiempo de ejecución, utilizar los **32 CPUs** disponibles en el nodo es la estrategia óptima, alcanzando 111 GFLOPS, a pesar de la reducción en eficiencia por hilo.
