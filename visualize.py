...
def plot_snapshot_3d(filename, save=False):
    ...
    if save:
        plt.savefig(f'snapshot_{snap_id:04d}.png', dpi=150, bbox_inches='tight')
        print(f"Saved snapshot_{snap_id:04d}.png")
    else:
        plt.show()

    plt.close()

def plot_energy_conservation(filename='contr.dat'):
    """
