...
void calc_force(int ni, int nj, double eps2, Predictor ipred[],
                Predictor jpred[], Force force[], double &, double &, double &);

#if 1
void calc_force(int ni, int nj, double eps2, Predictor ipred[],
                Predictor jpred[], Force force[], double &t1, double &,
                double &) {
  t1 = wtime();

  // Re-writing the loop with mask logic inside the replacement content
#pragma omp parallel for
  for (int i = 0; i < ni; i++) {
    ...
    double ix = ipred[i].pos.x;
    double iy = ipred[i].pos.y;
    double iz = ipred[i].pos.z;
    double ivx = ipred[i].vel.x;
    double ivy = ipred[i].vel.y;
    double ivz = ipred[i].vel.z;

#pragma omp simd reduction(+ : ax, ay, az, jx, jy, jz, pot)
    for (int j = 0; j < nj; j++) {
      double dx = jpred[j].pos.x - ix;
      double dy = jpred[j].pos.y - iy;
      double dz = jpred[j].pos.z - iz;
      double dvx = jpred[j].vel.x - ivx;
      double dvy = jpred[j].vel.y - ivy;
      double dvz = jpred[j].vel.z - ivz;

      double r2 = eps2 + dx * dx + dy * dy + dz * dz;

      // Mask out self-interaction (r2 == eps2 means distance is 0)
      // Using a small epsilon for float comparison safety, though exact
      // equality check was used before. Since we add eps2, r2 is always >=
      // eps2. If dist=0, r2=eps2.
      double mask = (r2 > eps2) ? 1.0 : 0.0;
